...
public class    CurioItemProperty extends AbstractTieredAccessory implements ICurioTexture{
    ...
    public Multimap<Attribute, AttributeModifier> getAttributeModifiers(SlotContext slotContext, UUID uuid, ItemStack stack){
        ...
        switch(gem){
            case NONE -> attribute.put(Attributes.ARMOR, new AttributeModifier(uuid, "bonus", 0.2 + (bonus * 0.05), AttributeModifier.Operation.ADDITION));
            case AMBER -> attribute.put(Attributes.KNOCKBACK_RESISTANCE, new AttributeModifier(uuid, "bonus", 0.025f + (bonus * 0.005), AttributeModifier.Operation.ADDITION));
            case EMERALD -> attribute.put(Attributes.LUCK, new AttributeModifier(uuid, "bonus", 1 + (bonus * 0.5), AttributeModifier.Operation.ADDITION));
            case RUBY -> attribute.put(Attributes.MAX_HEALTH, new AttributeModifier(uuid, "bonus", 0.5 + (bonus * 0.25), AttributeModifier.Operation.ADDITION));
            case SAPPHIRE -> attribute.put(Attributes.MOVEMENT_SPEED, new AttributeModifier(uuid, "bonus", 0.025 + (bonus * 0.015), Operation.MULTIPLY_TOTAL));
            case HEALTH -> attribute.put(Attributes.MAX_HEALTH, new AttributeModifier(uuid, "bonus", 1.5f + (bonus * 0.25), AttributeModifier.Operation.ADDITION));
            case WEALTH -> attribute.put(Attributes.LUCK, new AttributeModifier(uuid, "bonus", 1.5 + (bonus * 0.25), AttributeModifier.Operation.ADDITION));
            case TOUGH -> {
