    ...
    public Entity placeEntity(Entity entity, ServerLevel currentWorld, ServerLevel destinationWorld, float yaw, Function<Boolean, Entity> repositionEntity) {
        ...
        if (insideDimension) {
            ...
            for (BlockPos checkPos : BlockPos.betweenClosed(destinationPos.below(10).west(10),
                ...
                if (destinationWorld.getBlockState(checkPos).getBlock() instanceof ValoriaPortalBlock) {
                    doSetBlock = false;
                    break;
                }
            }
            ...
            if (doSetBlock) {
                ...
                ValoriaUtils.createEndShapedPortal(5, destinationWorld, blockpos1, BlockRegistry.VALORIA_PORTAL.get().defaultBlockState(), BlockRegistry.VALORIA_PORTAL_FRAME.get().defaultBlockState());
            }
        }
