package com.idark.valoria.registries.world.levelgen.feature;
...
public class AbyssalGlowfernFeature extends Feature<TwistingVinesConfig> {
        ...
        for(int i = 1; i <= pLength; ++i) {
            if (pLevel.isEmptyBlock(pPos)) {
                if (i == pLength || !pLevel.isEmptyBlock(pPos.above())) {
                    pLevel.setBlock(pPos, ModBlocks.ABYSSAL_GLOWFERN.get().defaultBlockState().setValue(GrowingPlantHeadBlock.AGE, Mth.nextInt(pRandom, pMinAge, pMaxAge)), 2);
                    break;
                }

                pLevel.setBlock(pPos, ModBlocks.ABYSSAL_GLOWFERN_PLANT.get().defaultBlockState(), 2);
            }
    ...
    private static boolean isInvalidPlacementLocation(LevelAccessor pLevel, BlockPos pPos) {
        if (!pLevel.isEmptyBlock(pPos)) {
            ...
            return !blockstate.is(ModBlocks.VOID_GRASS.get()) && !blockstate.is(ModBlocks.VOID_TAINT.get());
        }
    }
}
