...
@OnlyIn(Dist.CLIENT)
public class TinkeringScreen extends AbstractContainerScreen<TinkererMenu>{
    private final ResourceLocation BG_LOCATION = new ResourceLocation(Valoria.ID, "textures/gui/container/tinkering.png");
    ...
    protected void renderTooltip(GuiGraphics pGuiGraphics, int pX, int pY){
        ...
        if(this.displayRecipes){
            ...
            List<TinkeringRecipe> list = this.menu.getRecipes();
            for(int l = this.startIndex; l < k && l < this.menu.getNumRecipes(); ++l){
                ...
                if(pX >= j1 && pX < j1 + 16 && pY >= k1 && pY < k1 + 18){
                    ...
                    if(this.menu.getCurrentRecipe() != null && !this.menu.getCurrentRecipe().canCraft(this.menu.container)){
                        ...
                        TooltipRenderUtil.renderTooltipBackground(pGuiGraphics, pX - 110, pY - 10, Minecraft.getInstance().font.width(component) + 20, 15, 300, Color.BLACK.getRGB(), Pal.darkerGray.rgb(), Pal.darkishGray.rgb(), Pal.lightishGray.darker().rgb());
                        ms.translate(pX, pY, 300);
                        renderFloatingItem(pGuiGraphics, stack.copyWithCount(this.menu.getCurrentRecipe().getIngredientCount()), Minecraft.getInstance().font.width(component) - 107, k1 - 200);
                        pGuiGraphics.drawString(this.font, component, Minecraft.getInstance().font.width(component) - 190, k1 - 195, Color.WHITE.getRGB());
                        ms.popPose();
                    }
