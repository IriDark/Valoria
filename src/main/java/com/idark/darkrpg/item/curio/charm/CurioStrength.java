...
public class CurioStrength extends Item implements ICurioItem {
    ...
    public void curioTick(String identifier, int index, LivingEntity livingEntity, ItemStack stack) {
        ...
        if(!player.level.isClientSide()) {
            boolean hasPlayerFireResistance =
                    !Objects.equals(player.getEffect(Effects.DAMAGE_BOOST), null);

            if(!hasPlayerFireResistance) {
                player.addEffect(new EffectInstance(Effects.DAMAGE_BOOST, 230));
	...
	public void appendHoverText(ItemStack stack, World world, List<ITextComponent> tooltip, ITooltipFlag flags) {
		...
		tooltip.add(new TranslationTextComponent("tooltip.darkrpg.strength").withStyle(TextFormatting.GRAY));
	}
}